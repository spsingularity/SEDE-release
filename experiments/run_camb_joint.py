#!/usr/bin/env python3
"""
⚠️ DEPRECATED / SUPERSEDED — uses the λ=1 "temperature-factor" cousin (E_SEDE_H via
...
====================================================
Single engine (CAMB background, SEDE-H via w(a)) for ALL geometry + sound horizons:
...
Compares SEDE-H vs LCDM best-fit chi^2. Nelder-Mead (each eval ~0.05 s).
...
def main():
    print("=" * 70)
    print("PHASE 1: full joint with CAMB in the loop — SEDE-H vs LCDM")
    print("=" * 70)
    data = load()
    tH = build_or_load_table(model='sedeH'); iDH, iFH = fr.make_growth_interpolators(tH)
    tL = build_or_load_table(model='lcdm');  iDL, iFL = fr.make_growth_interpolators(tL)

    print("\nFitting SEDE-H (Om,H0,ombh2,gamma,M_B,sigma8)...")
    bH = fit('sedeH', data, iDH, iFH,
    ...
    bgH = Background(OmH, H0H, gH, obH, model='sedeH')
    OmL, H0L, obL, MBL, s8L = bL.x
    bgL = Background(OmL, H0L, 1.5, obL, model='lcdm')
    print("\n" + "-" * 70)
    print(f"SEDE-H: chi2={bH.fun:.2f}  Om={OmH:.3f} H0={H0H:.2f} 100ωb={obH*100:.3f} "
          f"γ={gH:.2f} σ8={s8H:.3f}")
    ...
    kH, kL = 6, 5
    print("\n" + "=" * 70)
    print(f"Δχ² (SEDE-H − LCDM) = {bH.fun - bL.fun:+.2f}   "
          f"ΔAIC = {(bH.fun+2*kH)-(bL.fun+2*kL):+.2f}   "
          f"({'SEDE-H' if bH.fun < bL.fun else 'LCDM'} preferred)")
