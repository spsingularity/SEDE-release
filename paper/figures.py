...
Run: python figures.py   (writes output/paper/fig_*.png)
"""
...
C = 299792.458                       # km/s
OUT = os.path.join(ROOT, "output/paper"); os.makedirs(OUT, exist_ok=True)
...
def fig1_mechanism():
    ...
    fig.tight_layout(); fig.savefig(f"{OUT}/fig1_mechanism.png"); plt.close(fig); print("fig1")
...
def fig2_inputs():
    ...
    a.set(xlabel=r"Barrow deformation $\Delta$", ylabel=r"$\lambda=1-\Delta/2$",
          title=r"$\lambda$ from the horizon (Thm 8–9)", xlim=(-.03,1.03), ylim=(.45,1.05))
    ...
    b.set(xlabel=r"entropy-weight exponent $p$", ylabel=r"$\gamma=d\ln\Sigma_S/d\ln\sigma_8$",
          title=r"$\gamma$ from structure (Thm 4C)")
    fig.suptitle("The dark sector is parameter-free: every input is derived", y=1.02, fontsize=11)
    fig.tight_layout(); fig.savefig(f"{OUT}/fig2_inputs.png", bbox_inches="tight"); plt.close(fig); print("fig2")
...
def fig3_datafit():
    ...
    fig.suptitle("The model fits the data: DESI BAO and Pantheon+ residuals vs ΛCDM", y=1.02, fontsize=11)
    fig.tight_layout(); fig.savefig(f"{OUT}/fig3_datafit.png", bbox_inches="tight"); plt.close(fig); print("fig3")
...
def _sede_w0wa():
    """SEDE's (w0,wa) by CPL-fitting w(z) over the DESI range (consistent with run_delta_indirect)."""
    zf = np.linspace(0, 1.5, 200); af = 1 / (1 + zf)
...
def fig4_eos():
    ...
    fig.suptitle("SEDE's phantom-crossing EOS, alongside the official DESI DR2 w0waCDM result", y=1.02, fontsize=11)
    fig.tight_layout(); fig.savefig(f"{OUT}/fig4_eos.png", bbox_inches="tight"); plt.close(fig); print("fig4")
...
def fig5_oos():
    ...
    ax.text(0.02,0.03,"held-out DESI Δχ²(SEDE−ΛCDM) = −8.2", transform=ax.transAxes,
            fontsize=8.5, bbox=dict(boxstyle="round", fc="white", ec="0.7"))
    ax.legend(fontsize=8.5, loc="upper right")
    fig.tight_layout(); fig.savefig(f"{OUT}/fig5_oos.png", bbox_inches="tight"); plt.close(fig); print("fig5")
...
def fig6_forecast():
    ...
    ax.set(ylabel=r"separation of $\Delta=1$ from $\Delta=0$  [$\sigma$]",
           title="The decisive test: measuring the Barrow deformation Δ", ylim=(0,13))
    ax.text(0.02,0.95,"Δ=1 (volume-law horizon) vs Δ=0 (smooth):\nDR3+Euclid settle it",
            transform=ax.transAxes, va="top", fontsize=8.5,
            bbox=dict(boxstyle="round", fc="white", ec="0.7"))
    fig.tight_layout(); fig.savefig(f"{OUT}/fig6_forecast.png", bbox_inches="tight"); plt.close(fig); print("fig6")
