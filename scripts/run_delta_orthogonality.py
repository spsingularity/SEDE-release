...
This script computes Ω_DE(z=3) and w₀ vs Δ for both, and makes output/fig_delta_orthogonality.png.
...
def main():
    ...
    ax[0].annotate(f'SEDE Δ=1: {s1:.3f}\n≲ ΛCDM early-DE\n(independently CMB-safe)',
                   xy=(1.0, s1), xytext=(0.42, 0.20), fontsize=7.5, color='C0',
                   arrowprops=dict(arrowstyle='->', color='C0', lw=1))
    ax[0].annotate(f'Barrow-HDE Δ=1: {b1:.3f}\n(5×, CMB-disfavoured)',
                   xy=(1.0, b1), xytext=(0.30, 0.40), fontsize=7.5, color='C3',
    ...
    ax[0].set_title('The gate suppresses the early-DE the CMB constrains:\nSEDE Δ=1 is CMB-safe, Barrow-HDE Δ=1 is not')
    ...
    ax[1].set_xlabel('preferred Δ in each model'); ax[1].set_title('Same data, ORTHOGONAL Δ preferences\n(the gate is the difference)')
    ax[1].legend(fontsize=8, loc='upper center')
    plt.tight_layout(); plt.savefig('output/fig_delta_orthogonality.png', dpi=130)
    ...
    print("  saved -> output/fig_delta_orthogonality.png")
